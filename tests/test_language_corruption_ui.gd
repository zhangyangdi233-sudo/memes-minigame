...
func _run_async() -> void:
	...
	if scene != null:
		...
		_assert_true(_find_node_by_name(root, "HUDMoneyIcon") == null, "money must not remain a visible progress metric")
		var exit_button := _find_node_by_name(root, "ExitGameButton") as Button
		_assert_true(exit_button != null and exit_button.visible, "退出游戏 should remain visible during play")
		if exit_button != null:
		...
		if root.has_method("_request_quit_game"):
			...
			_assert_true(message != null and message.text == "别把我一个人留在这里。", "the exit plea should use the exact required sentence")
